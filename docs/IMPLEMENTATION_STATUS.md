...
This is the compact execution index for the [road to v1](UX_VISION.md#16-road-to-v1). “Implemented” means the code and automated contract exist; proprietary map behavior still requires the linked Windows/game-client test.
...
| 0.8 route corpus | Next | Not implemented | Add deterministic outdoor/indoor/stacked/ramp/door/drop fixtures and expected outcomes |
...
Build the route-corpus test harness before replacing or augmenting the existing pathfinder. The corpus must establish expected outcomes and directed elevation rules independently of any prospective navmesh library, otherwise a new pathfinder cannot be evaluated objectively.

Acceptance for that slice:

- synthetic redistributable fixtures cover outdoor, narrow indoor, stacked floors, a legal ramp, a blocked door/wall, an exposed drop, and an illegal ascent over +6 EQ Z;
...
- [UX_VISION.md](UX_VISION.md), [HANDOFF.md](HANDOFF.md), [VALIDATION.md](../VALIDATION.md), and CHANGELOG are updated with the result and next exact task.
